...
class CircularDeque:
    ...
    def grow(self) -> None:
        """
        Doubles the capacity of circular deque
        """
        old = self.queue
        self.capacity *= 2
    ...
    def shrink(self) -> None:
        """
        Cuts the capacity of circular dequeue in half
        """
        old = self.queue
    ...
    def enqueue(self, value: T, front : bool = True) -> None:
        ...
        :Param value: Value to add to CD
        :Param front: If true, add to front. False, add to back
        """
        if self.is_empty():
            self.front = 0
            self.back = 0
    ...
    def dequeue(self, front : bool = True) -> T:
        """
        Remove an item from the front or back of the CD
        :param front: Removes from the front if true, back if false
...
class CDLLCD:
    ...
    def enqueue(self, val: T, front: bool = True) -> None:
        ...
        :param front: add to front if true, add to back if false
        """
    ...
    def dequeue(self, front: bool = True) -> T:
        ...
        :param front: remove from front if true, back if false
        :return: value removed
        """
        if self.CDLL.head is None:
            return None

        if front:
            answer = self.CDLL.head.next.val

        else:
            answer = self.CDLL.head.prev.val
